...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.cfg = {"MODEL": {"SparseRCNN": {"NHEADS": 8, "DROPOUT": 0.0, "DIM_FEEDFORWARD": 2048, "ACTIVATION": 'relu',
                                             "HIDDEN_DIM": self.d_model,
                                             "NUM_CLS": 1, "NUM_REG": 3, "NUM_HEADS": 6, "NUM_DYNAMIC": 2,
                                             "DIM_DYNAMIC": 64
                                             },
                              "ROI_BOX_HEAD": {
                                  'POOLER_RESOLUTION': 7,
        ...
        # QRF Module  查询和RoI融合
        self.dynamic_layer_for_current_query1 = RCNNHead(self.cfg, d_model, num_classes, dim_feedforward, nhead,
                                                         dropout, activation)
        self.dynamic_layer_for_current_query2 = RCNNHead(self.cfg, d_model, num_classes, dim_feedforward, nhead,
                                                         dropout, activation)
        self.dynamic_layer_for_current_query3 = RCNNHead(self.cfg, d_model, num_classes, dim_feedforward, nhead,
                                                         dropout, activation)

        # TDTD Module 时间不规则TF解码器
    ...
    def forward(self, srcs, masks, pos_embeds, imgs_whwh_shape, query_embed=None, class_embed=None, cur_bbox_embed=None,
                temp_class_embed_list=None, temp_bbox_embed_list=None):
        ...
        # 对输入的每个级别进行形状变换和拆分，以便后续的编码器处理。
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            bs, c, h, w = src.shape  # src和pos_embed为Tensor(15,256,71,125)，mask为(15,71,125)
            feats_whwh = (w, h, w, h)
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)
            src = src.flatten(2).transpose(1, 2)  # (15,8875=71*125,256)
            mask = mask.flatten(1)  # (15,8875)
            pos_embed = pos_embed.flatten(2).transpose(1, 2)  # (15,8875,256)
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)  # (15,8875,256)
            lvl_pos_embed_flatten.append(lvl_pos_embed)
            src_flatten.append(src)  # swinB只有一个元素
            mask_flatten.append(mask)

        # 对一些张量进行拼接和转换，以准备进行后续的编码器和解码器操作。
        src_flatten = torch.cat(src_flatten, 1)  # (15,8875,256) 沿张量第一个维度进行拼接
        mask_flatten = torch.cat(mask_flatten, 1)  # (15,8875)
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)  # (15,8875,256)
        ...
        # 论文中的STE，encoder，调用DeformableTransformerEncoder.forward()
        ...
        # 论文中的STD，decoder，调用DeformableTransformerDecoder.forward()
        ...
        if self.TDAM:
            ...
            frame_start_index = torch.cat(
                (ref_spatial_shapes.new_zeros((1,)), ref_spatial_shapes.prod(1).cumsum(0)[:-1])).contiguous()
            ...
            # 对当前帧应用论文中的QRF模块：Query and RoI Fusion (QRF) of current frame
            ...
            # ===========第一阶段的精炼
            topk_values, topk_indexes = torch.topk(ref_prob_concat.view(ref_hs_logits_concat.shape[0], -1),
                                                   80 * self.num_ref_frames, dim=1)
            topk_indexes = topk_indexes // ref_hs_logits_concat.shape[2]
            ref_hs_input1 = torch.gather(ref_hs_concat, 1,
                                         topk_indexes.unsqueeze(-1).repeat(1, 1, ref_hs_concat.shape[-1]))

            # 论文中的TQE模块
            cur_hs = self.temporal_query_layer1(cur_hs, ref_hs_input1)

            # 论文中的TDTD模块
            cur_hs, cur_references_out = self.temporal_decoder1(cur_hs, cur_reference_out, cur_memory,
                                                                spatial_shapes[0:1], level_start_index[0:1],
                                                                valid_ratios[0:1], None, None)
            ...
            out['aux_outputs'] = [{"pred_logits": output_class1, "pred_boxes": output_coord1}]  # 保存预测

            # =======第二阶段的精炼
            topk_values, topk_indexes = torch.topk(ref_prob_concat.view(ref_hs_logits_concat.shape[0], -1),
                                                   50 * self.num_ref_frames, dim=1)
            topk_indexes = topk_indexes // ref_hs_logits_concat.shape[2]
            ref_hs_input2 = torch.gather(ref_hs_concat, 1,
                                         topk_indexes.unsqueeze(-1).repeat(1, 1, ref_hs_concat.shape[-1]))
            ...
            topk_indexes = topk_indexes // ref_hs_logits_concat.shape[2]
            ref_hs_input3 = torch.gather(ref_hs_concat, 1,
                                         topk_indexes.unsqueeze(-1).repeat(1, 1, ref_hs_concat.shape[-1]))
            cur_hs = self.temporal_query_layer3(cur_hs, ref_hs_input3)
            # 产生最终预测 final_hs(1,100,256)  final_references_out(1,100,4)
            final_hs, final_references_out = self.temporal_decoder3(cur_hs, cur_reference_out, cur_memory,
                                                                    spatial_shapes[0:1], level_start_index[0:1],
                                                                    valid_ratios[0:1], None, None)

            return hs[:, 0:1, :, :], init_reference_out[0:1], inter_references_out[:, 0:1, :, :], None, None, \
...
class TemporalDeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, frame_start_index,
...
class DeformableTransformerEncoder(nn.Module):
    """对应论文中的STE，包括6个DeformableTransformerEncoderLayer"""
